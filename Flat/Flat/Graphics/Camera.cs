    ...
    public sealed class Camera
    {
        ...
        public void Update()
        {
            ...
            if (ShakeDuration > 0f)
            {
                ...
                if (t - ShakeTimer > ShakeInterval)
                {
                    ShakeTimer = t;
                    Vector2 offset;
                    offset.X = (RandomHelper.RandomSingle() * 2 - 1) * ShakeMagnitude.X;
                    offset.Y = (RandomHelper.RandomSingle() * 2 - 1) * ShakeMagnitude.Y;
                    Move(offset);
                    Shakeoffset += offset;
                }
            ...
            this.view = Matrix.CreateLookAt(new Vector3(position, (float)this.z), new Vector3(position,0f), new Vector3(this.up, 0f));
            this.proj = Matrix.CreatePerspectiveFieldOfView(this.fieldOfView, this.aspectRatio, Camera.MinZ, Camera.MaxZ);

            this.updateRequired = false;
        }

        public void Shake(float shakeduration, Vector2 shakeMagnitude, float shakeinterval)
        {
            if (this.ShakeDuration > 0f) return;
